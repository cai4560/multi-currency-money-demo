...
public class DollarTest {
    ...
    public void test_one_dollar_add_one_dollar_should_equal_two_dollar() {
        ...
        assertEquals(one.plus(anotherOne).reduce(bank, Money.DOLLAR_CURRENCY), Money.dollar(2));
    }
    ...
    public void test_one_dollar_add_two_dollar_should_equal_three_dollar() {
        ...
        assertEquals(one.plus(two).reduce(bank, Money.DOLLAR_CURRENCY), Money.dollar(3));
    }
    ...
    public void test_one_dollar_reduce_one_dollar() {
        ...
        assertEquals(oneDollar, oneDollar.reduce(bank, Money.DOLLAR_CURRENCY));
    }

    @Test
    public void test_two_dollar_reduce_two_dollar() {
        Money twoDollar = Money.dollar(2);
        assertEquals(twoDollar, twoDollar.reduce(bank, Money.DOLLAR_CURRENCY));
    }

    @Test
    public void test_one_dollar_reduce_two_franc() {
        Money oneDollar = Money.dollar(1);
        assertEquals(Money.franc(2), oneDollar.reduce(bank, Money.FRANC_CURRENCY));
    }

    @Test
    public void test_one_dollar_add_two_franc_equals_two_dollar() {
        Expression sum = new Sum(Money.dollar(1), Money.franc(2));
